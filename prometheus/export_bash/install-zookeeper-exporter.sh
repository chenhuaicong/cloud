...
cd  ${installdir}/ && wget http://soft.example.com/prometheus/zookeeper-exporter
...
#	rm -f /lib/systemd/system/zookeeper-exporter.service
	curl --request PUT  http://consul.example.com.cn/v1/agent/service/deregister/$(hostname)-${ipaddr}-zookeeper-exporter
else
