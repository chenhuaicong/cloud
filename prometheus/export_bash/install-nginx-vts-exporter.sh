...
cat ${installdir}/nginx-V|grep "nginx-module-vts" >/dev/null
        if [ "$?" -ne 0 ];then
                ...
                echo "下载地址：http://soft.example.com/prometheus/nginx-module-vts.tar"
                exit  10
        fi
...
eof
        ...
        cd  ${installdir} && wget http://soft.example.com/prometheus/nginx-exporter
...
#       rm -f /data/1180nginx/vhost/nginx-exporter.conf
        curl --request PUT  http://consul.example.com.cn/v1/agent/service/deregister/$(hostname)-${ipaddr}-nginxexporter
else
